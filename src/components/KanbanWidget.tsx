...
const COLUMN_COLORS: Record<string, { header: string; border: string }> = {
  gray:   { header: 'bg-gray-100',  border: 'border-[var(--sh-b)]'   },
...
export function KanbanWidget({ component, data: canvasData, runtime, onEvent }: WidgetProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Toolbar */}
      {showSearch && (
        <div className="flex items-center gap-3 rounded-lg border border-[var(--sh-b)] bg-[var(--sh)] px-4 py-2.5">
          <div className="relative flex-1">
            <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--sh-td)] pointer-events-none" />
            <input
              type="text"
              value={search}
              ...
              className="w-full rounded-md border border-[var(--sh-b)] bg-[var(--sh)] py-1.5 pl-8 pr-3 text-sm placeholder:text-[var(--sh-td)] focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            />
          </div>
          <span className="text-xs text-[var(--sh-td)] tabular-nums">
          ...
          return (
            ...
            >
              ...
              <div className={clsx(
                ...
                <h3 className="text-sm font-semibold text-[var(--sh-ts)]">{column.title}</h3>
                <div className="flex items-center gap-1.5">
                  <span className="rounded bg-[var(--sh)] px-2 py-0.5 text-xs font-medium text-[var(--sh-t)] tabular-nums">
                    {total}
                  </span>
                  {column.limit != null && (
                    <>
                      <span className={clsx(
                        ...
                        isFull ? 'text-red-500' : 'text-[var(--sh-td)]',
                      )}>
                        / {column.limit}
                      </span>
                      {isFull && (
              ...
              <div className="flex min-h-[200px] flex-col gap-2 p-3">
                {visible.length === 0 ? (
                  <div className="flex flex-1 items-center justify-center py-8 text-xs text-[var(--sh-td)]">
                  ...
                  visible.map(item => {
                    ...
                    return (
                      <div
                        ...
                        className="cursor-grab rounded-md border border-[var(--sh-b)] bg-[var(--sh)] p-3 shadow-sm transition-shadow hover:shadow-md active:cursor-grabbing active:opacity-70"
                      >
                        ...
                        {desc && (
                          <p className="mt-1.5 line-clamp-2 text-xs text-[var(--sh-t)]">{desc}</p>
                        )}

                        {(assignee || dueDate) && (
                          <div className="mt-2 flex items-center justify-between text-xs text-[var(--sh-td)]">
